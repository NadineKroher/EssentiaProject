...
if viz ~= 0 & ndim == 1
  ...
  [histn, histx] = hist(x(:,1),1000);
%  vscale = max(histn);
% Scale is wrong
histn = histn/size(x,1) /((histx(1000)-histx(1))/999); % change to density
vscale = 1/sqrt(2*pi);%*(histx(1000)-histx(1))/999; % Rectified by Zhiyao Duan, 10/07/2007
end
...
for it = 1:its
  ...
  if viz == 1
    if ndim == 1
      bar(histx, histn);
      gmmplot(mo,vo,co,[1],vscale,'r'); % Rectified by Zhiyao Duan, 10/07/2007
...
if viz == -1
  if ndim == 1
    bar(histx, histn, 'FaceColor', [0.5,0.5,0.5]);
    gmmplot(mo,vo,co,[1],vscale,'r'); % Rectified by Zhiyao Duan, 10/07/2007
%    gmmplot(mo,vo,co,[1],vscale*min(sqrt(vo)./co),'r');
  else
    plot(x(:,1),x(:,2),'.b');
